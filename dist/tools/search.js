...
export function registerSearchTool(server, api) {
    server.registerTool('search_files', {
        title: 'Search Files',
        description: 'Search enrolled files by name. Returns matching files with their ' +
        ...
        try {
            ...
            const summary = files
                .map((f) => `- **${f.name}** (id: ${f.platformId}, type: ${f.fileType})` +
    ...
    server.registerTool('get_unattributed_changes', {
        ...
        description: 'Get pending cell-level changes that have not been attributed to a ' +
        ...
        try {
            ...
            const summary = changes
                .map((c) => `- **${c.sheetName}!${c.cellAddress}** (${c.changeType}): ` +
